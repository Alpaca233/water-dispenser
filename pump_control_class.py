...
class PumpController:
    ...
    def _initialize_pump(self):
        ...
        try:
            ...
            self.client.write_register(
                self.REG_COMMUNICATION, self.COMM_RS485, unit=self.unit_id
            )
            ...
            self.client.write_registers(
                self.REG_MAX_RPM, [0, self.max_rpm * 100], unit=self.unit_id
            )
    ...
    def run(self, rpm, duration=None, reverse=False):
        ...
        if rpm > self.max_rpm:
            print(
                f"Requested RPM ({rpm}) exceeds max RPM ({self.max_rpm}). Setting to max."
            )
            rpm = self.max_rpm
        ...
        try:
            ...
            self.client.write_register(self.REG_DIRECTION, direction, unit=self.unit_id)

            # Set target RPM (multiply by 100 as per protocol)
            self.client.write_registers(
                self.REG_TARGET_RPM, [0, rpm * 100], unit=self.unit_id
            )

            time.sleep(0.1)

            # Start pump
            self.client.write_coil(self.COIL_START_STOP, True, unit=self.unit_id)
...
class SimulatedPumpController:
    ...
    def run(self, rpm, duration=None, reverse=False):
        ...
        if rpm > self.max_rpm:
            print(
                f"[SIMULATED] Requested RPM ({rpm}) exceeds max RPM ({self.max_rpm}). Setting to max."
            )
            rpm = self.max_rpm
        ...
        try:
            ...
            print(
                f"[SIMULATED] Pump started: {rpm} RPM, {direction_str}, {duration_str} duration"
            )
    ...
    def get_status(self):
        ...
        """
        runtime = (
            time.time() - self.start_time if self.start_time and self.is_running else 0
        )
        return {
